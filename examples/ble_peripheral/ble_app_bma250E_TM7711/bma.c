...
//	twi_master_init();
  ...
  SCL_();
	...
	bma255_set_bandwidth(BMA255_BW_125HZ);
...
//	BMA250E_WriteReg(BMA255_TAP_THRES_REG,0xc1); //设置触发阈值
	...
	BMA250E_WriteReg(BMA255_SLOPE_THRES_REG, 0x01);//阈值寄存器设置 2g--3.91mg--0x14 4g--7.81mg 8g--15.63mg--0x0f 16g--31.25mg
...
//	BMA250E_ReadReg_DMWZ(0x00,buf,1);
	...
	unsigned char buf_bma[4]={0,0,0,0};
	unsigned char _wakeup_=0x12;
...
//任意运动检测
	buf_bma[0]=0;
	BMA250E_WriteReg(BMA255_INT_CTRL_REG, 0x03);//设置中断为锁死模式
	BMA250E_ReadReg( BMA255_INT_CTRL_REG,buf_bma);
	if(buf_bma[0]!=0x03){success_flag=1;}
	
	buf_bma[0]=0;
	BMA250E_WriteReg(BMA255_SLOPE_THRES_REG, _wakeup_);//阈值寄存器设置 2g--3.91mg--0x14 4g--7.81mg 8g--15.63mg--0x0f 16g--31.25mg
	BMA250E_ReadReg( BMA255_SLOPE_THRES_REG,buf_bma);
	if(buf_bma[0]!=0x12){success_flag=1;}
	
	buf_bma[0]=0;
	BMA250E_WriteReg(BMA255_SLOPE_DURN_REG, 0x00);//当斜率大于阈值，超过slope_dur+1的时间，产生中断
	BMA250E_ReadReg( BMA255_SLOPE_DURN_REG,buf_bma);
	if(buf_bma[0]!=0x00){success_flag=1;}
	
	buf_bma[0]=0;
	BMA250E_WriteReg(BMA255_INT1_PAD_SEL_REG, 0x04);//设置中断引脚init1
	BMA250E_ReadReg( BMA255_INT1_PAD_SEL_REG,buf_bma);
	if(buf_bma[0]!=0x04){success_flag=1;}
	
	buf_bma[0]=0;
	BMA250E_WriteReg( BMA255_INT_ENABLE1_REG,0x07); //中断使能设置
		BMA250E_ReadReg(BMA255_INT_ENABLE1_REG,buf_bma);
	if(buf_bma[0]!=0x07){success_flag=1;}

	buf_bma[0]=0;
	//任意运动检测end
	BMA250E_WriteReg(BMA255_MODE_CTRL_REG, 0x56);//设置low power使能
	BMA250E_ReadReg(BMA255_MODE_CTRL_REG,buf_bma);
	if(buf_bma[0]!=0x56){success_flag=1;}
	
	if(num>250){NVIC_SystemReset();}
	num++;
...
////任意运动检测
//	BMA250E_WriteReg(BMA255_INT_CTRL_REG, 0x03);//设置中断为锁死模式
//	BMA250E_WriteReg(BMA255_SLOPE_THRES_REG, 0x12);//阈值寄存器设置 2g--3.91mg--0x14 4g--7.81mg 8g--15.63mg--0x0f 16g--31.25mg
//	BMA250E_WriteReg(BMA255_SLOPE_DURN_REG, 0x00);//当斜率大于阈值，超过slope_dur+1的时间，产生中断
//	BMA250E_WriteReg(BMA255_INT1_PAD_SEL_REG, 0x04);//设置中断引脚init1
//	BMA250E_WriteReg(BMA255_INT_ENABLE1_REG,0x07); //中断使能设置
//	//任意运动检测end
...
//	BMA250E_WriteReg(BMA255_MODE_CTRL_REG, 0x56);//设置low power使能
//	BMA250E_ReadReg(BMA255_MODE_CTRL_REG,buf_bma);
//	while(buf_bma[0]!=0x56&&buf_bma[1]<=3){
//	BMA250E_WriteReg(BMA255_MODE_CTRL_REG, 0x56);//设置low power使能
//	BMA250E_ReadReg(BMA255_MODE_CTRL_REG,buf_bma);
//	buf_bma[1]++;
//	}
